...
pub async fn backup_to(source: PathBuf, path: Option<String>, server: Server) -> Result<(), Box<dyn std::error::Error>> {
    match server {
        Server::S3(s3server) => {
            ...
            backup_to_s3(source, path, s3server).await?
        }
        Server::SSH(ssh_server) => {
            debug!("开始推送到ssh服务器:{}",&ssh_server.name);
            backup_to_ssh(source, path, ssh_server).await?;
        }
    };
    Ok(())
}
...
pub async fn backup_to_s3(source: PathBuf, path: Option<String>, s3server: S3Server) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 构造对象键
    let destination_key = if let Some(dir) = path.or_else(|| Some(s3server.default_path.clone())) {
...
pub async fn backup_to_ssh(source: PathBuf, path: Option<String>, ssh_server: SSHServer) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let remote_path = format!("{}/{}", path.unwrap_or_else(|| ssh_server.default_path.clone()), source.file_name().unwrap().to_str().unwrap());
